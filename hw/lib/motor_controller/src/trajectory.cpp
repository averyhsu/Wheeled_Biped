...
void rotate(Motor device, int degrees){
        //1) 360 degrees in 3000 millisecond
    //2) 1)--> 0.12 deggrees / ms --> 12 (0.01 degrees)/ms
    //3) send frequency = 1ms
    const int DEGREES = degrees*100; //360 degrees = 36000 (0.01 degrees)
    const int STEP_WIDTH = 50; //12 degrees = 1200 (0.01 degrees)
    const int DELAY = 3000; //in micros: 1 milli = 1000 micros 1kHZ speed
...
void go_to(Leg leg, double x, double y, int speed){
    ...
    leg.motor1.write(pos_abs, angles.theta1, speed);
    leg.motor2.write(pos_abs, angles.theta2, speed);
}
...
void circle (Leg leg, double radius){
    ...
    const int FREQ = 10000; //unit: microseconds; --> 1 millisecond = 1khz
...
void square(Leg leg, double halfSide){
    ...
    const uint32_t LOOP_TIME = int(TOTAL_STEPS * (FREQ / 1000.0));
